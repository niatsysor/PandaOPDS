...
**OPDS-PSE 串流服务器**：E-Hentai.org 的中转代理，输出 OPDS 1.2 Atom 目录 + OPDS-PSE 串流链接，服务自研阅读器客户端（对标 Panels）。
...
| `GET /opds/search.xml` | OpenSearchDescription |
| `GET /opds/gallery?query=&next=` | 图库采集 feed（`rel="next"` 分页） |
| `GET /opds/gallery/{gid}/{token}/chapters` | 章节 feed（单条目 + PSE stream link） |
...
自研阅读器把 OPDS 根 URL（如 `https://opds.example.com/opds`）加入目录源即可：

1. 根 feed → Latest / Popular / Search 子目录
2. 图库 feed → 条目带 acquisition link 指向章节 feed
3. 章节 feed → 单条目含 PSE stream link：
   `href="/stream/{gid}/{token}/page/{pageNumber}"`，`pse:count` 为页数
4. 客户端把 `{pageNumber}` 替换为 1..count（默认 1-based；`PSE_PAGE_BASE=0` 时为 0..count-1）逐页拉流
